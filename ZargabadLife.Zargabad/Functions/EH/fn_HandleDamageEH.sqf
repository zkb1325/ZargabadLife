...
if (_projectile in ["CUP_B_9x18_SD","CUP_B_12Gauge_74Slug"]) then
	{
	if ((((_projectile isEqualTo "CUP_B_12Gauge_74Slug") and (_unit distance _instigator <= 50)) or ((_projectile isEqualTo "CUP_B_9x18_SD") and (_unit distance _instigator <= 15)))) then
		{
		if (_unit getVariable ["knockedOut",false]) exitWith {};
		if !(isNil "ZKB_Stunned") exitWith {};
		ZKB_Stunned = true;
		if ((side _unit isEqualTo west) and (side _instigator isEqualTo west)) exitWith {};
